 ...
 */
const useAssetBatch = (pagination) => {
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState(null)
  const [assetBatchList, setAssetBatchList] = useState([])
  const [assetBatchTypeList, setAssetBatchTypeList] = useState([])
  ...
  const fetchCodeList = useCallback(async () => {
    try {
      ...
      if (resApi.success) {
        setAssetBatchTypeList(resApi.data)
      } else {
        setAssetBatchTypeList([])
      }
    } catch (error) {
   ...
   * 자산 일괄 처리 목록 조회
   */
  const fetchAssetBatchFilter = useCallback(
    async (mbrNoParam) => {
      if (!mbrNoParam) {
        console.warn('fetchAssetBatchFilter called without mbrNoParam')
        return
      }
      ...
      try {
        ...
        if (resApi && resApi.success && resApi.data) {
          setAssetBatchList(resApi.data.items || [])
        ...
        console.error('fetchAssetBatchFilter error:', error)
        setError(error)
        setAssetBatchList([])
      } finally {
        setIsLoading(false)
